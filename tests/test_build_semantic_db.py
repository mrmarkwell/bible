...
class TestBuildSemanticDb(unittest.TestCase):
    ...
    def test_main_dry_run_corpus_4(self):
        out = io.StringIO()
        with patch("sys.stdout", out):
            code = main(["--db", str(DEFAULT_DB_PATH), "--dry-run", "--corpus", "4"])
        self.assertEqual(code, 0)
        val = out.getvalue()
        self.assertIn("[Dry Run]", val)
        self.assertIn("Pastoral & General Epistles", val)

    def test_main_dry_run_corpus_5(self):
        out = io.StringIO()
        with patch("sys.stdout", out):
            code = main(["--db", str(DEFAULT_DB_PATH), "--dry-run", "--corpus", "5"])
        self.assertEqual(code, 0)
        val = out.getvalue()
        self.assertIn("[Dry Run]", val)
