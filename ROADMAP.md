...
- [x] **Task 7.7**: Implement Semantic Passport Generator & Batch Vector Ingestion Engine (`tools/build_vector_db.py` / `./bible build-vectors` per ADR-083) with `--corpus`, `--book`, and `--resumable` checkpoint ledger support, `batchEmbedContents` integration, and signed `int8` quantization. *(Run 092 / ADR-100)*
- [x] **Task 7.8**: Whole-Bible Vector Database Campaign: Corpus 1 - Foundational Pauline Epistles & Hebrews (Romans, 1-2 Corinthians, Galatians, Ephesians, Philippians, Colossians, Hebrews; 115 pericopes) via `./bible build-vectors --corpus 1`. *(Run 097 / ADR-105)*
- [x] **Task 7.9**: Whole-Bible Vector Database Campaign: Corpus 2 - The Four Gospels & Acts (Matthew, Mark, Luke, John, Acts; 148 pericopes) via `./bible build-vectors --corpus 2`. *(Run 098 / ADR-106)*
...
- [x] **Task 7.14**: Whole-Bible Vector Database Campaign: Corpus 7 - Historical Books & Apocalyptic Consummation (Joshua to Esther, Revelation; 285 pericopes) via `./bible build-vectors --corpus 7`. *(Run 104 / ADR-112)*
- [x] **Task 7.15**: Whole-Bible Verse-Level Fine-Grained Micro-Anchor Embedding Ingestion (~31,102 verses mapped to parent pericopes). *(Run 106 / ADR-114 — Phase 7 100% Complete & All 9 Phases 100% Complete)*
...
- [x] **Task 9.1**: Implement author-scoped dynamic per-turn RAG retrieval hook in `core/persona.py` (`CharacterDialogueSession.step()`) so characters dynamically pull their relevant canonical passages with similarity scores into dialogue context. *(Run 108 / ADR-116)*
- [x] **Task 9.2**: Implement canonical "Whole Bible Counselor" (`whole-bible`) persona in `core/persona.py` and enforce mandatory Scripture citation formatting (`[Book Chapter:Verse]`) with split-screen reader hyperlinks across all persona responses. *(Run 109 / ADR-117)*
- [x] **Task 9.3**: Implement dedicated Semantic Concordance & Vector Similarity UI panel in Web UI (`web/static/index.html`, `web/static/app.js`) and REST endpoint (`/api/similar?q=...`), rendering ranked passages with percentage match strength badges. *(Run 112 / ADR-120)*
- [x] **Task 9.4**: Implement Web UI "Ask the Bible" Theological Inquiry Studio, rendering synthesized RAG answers side-by-side with retrieved pericopes, typological links, and reciprocal rank fusion scores. *(Run 113 / ADR-121 — Phase 9 100% Complete & All 10 Phases 100% Complete)*
