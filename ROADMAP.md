...
- [ ] **Task 7.7**: Offline Whole-Bible Context-Enriched Pericope & Verse Vector Database Generation (ADR-076, ADR-083). Pre-compute dense vector embeddings offline using standard embedder `text-embedding-004` (task_type=`RETRIEVAL_DOCUMENT`) for all 1,304 pericopes (enriched with the 3-tier Semantic Passport: Book Horizon, genre, theological loci, thematic ribbons, preceding discourse, and central propositions) and 31,102 verses, storing signed `int8` quantized BLOBs in SQLite (`pericope_embeddings` and `verse_embeddings` tables in `data/bible.db`) for permanent zero-dependency offline similarity search.
- [ ] **Task 7.8**: Semantic Passport Generator & Batch Vector Ingestion Pipeline (`tools/build_vector_db.py` / `./bible build-vectors` per ADR-083). Build zero-dependency CLI batch compilation engine to assemble structured pericope Semantic Passports, batch-embed via Gemini `text-embedding-004`, quantize to `int8` packed bytes, and commit to SQLite with resumable checkpoint ledger.
